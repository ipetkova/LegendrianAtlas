...
# A pair of functions that find all the the pinches (depinches) possible from a given grid
# Some speedups are implemented here, I can explain them in a writeup?
def generated_pinched_grids(grid):
    X,O = grid[0],grid[1]
    n = len(X)
    new_grids = set()
...
def generated_birthed_grids(grid):
    X,O = grid[0],grid[1]
    n = len(X)
    new_grids = set()
